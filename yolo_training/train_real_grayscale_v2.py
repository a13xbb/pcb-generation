#!/usr/bin/env python3
"""Train YOLOv8 on real PCB dataset with on-the-fly augmentations.

Per image, per epoch (training only):
  1. With prob binarize_p → binarize_real (two-step Otsu, traces white / background black)
     else → grayscale
...
Usage:
    python train_real_grayscale_v3.py
    python train_real_grayscale_v3.py --binarize_p 0.3 --crop_p 0.3 --epochs 100
    python train_real_grayscale_v3.py --preview
"""
...
from augmentations import to_grayscale, binarize_real, sample_crop_params, apply_crop_zoom

ROOT = Path(__file__).parent.parent
DEFAULT_DATA = ROOT / "datasets/pcb-defect-dataset/data_synth_aligned.yaml"
OUT = ROOT / "yolo_training/results"


def parse_args():
    p = argparse.ArgumentParser(description="Train YOLOv8 on real PCB with gray/binary/crop augmentation")
    ...
    p.add_argument("--name", default="real_grayscale_v3")
    p.add_argument("--preview", action="store_true",
                   help="Save augmentation examples to debug_output/real_v3_preview and exit")
...
def save_preview(data_yaml: Path, out_dir: Path, n_images: int,
    ...
    src_path = cfg_path if cfg_path.is_absolute() else (data_yaml.parent / cfg_path).resolve()
    train_val = cfg.get("train", "images")

    # Handle txt file list vs directory
    all_imgs = []
    if isinstance(train_val, str) and train_val.endswith(".txt"):
        ...
        if txt_path.exists():
            for line in txt_path.read_text().splitlines():
                ...
                if p.exists():
                    ...
                    alt = src_path / "train" / "images" / Path(line).name
                    if alt.exists():
                        all_imgs.append(alt)
    else:
        img_dir = src_path / train_val
        if not img_dir.is_dir():
            img_dir = src_path / "train" / "images"
    ...
    chosen = sorted(all_imgs)[:n_images]
...
class RealPCBDataset(YOLODataset):
    """YOLODataset with on-the-fly grayscale/binarize_real and zoom-in crop augmentation."""
...
class RealPCBTrainer(DetectionTrainer):
    """DetectionTrainer using RealPCBDataset for on-the-fly real-PCB augmentations."""
...
def main():
    ...
    if args.preview:
        save_preview(
            Path(args.data),
            ROOT / "debug_output" / "real_v3_preview",
    ...
    print("=" * 60)
    print("Real PCB Training — on-the-fly gray/binary/crop augmentation (v3)")
    print("=" * 60)
